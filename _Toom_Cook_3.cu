...
// Multiply
__global__ void _Toom_Cook_3::Multiply() 
{
	Split();
	Evaluation();
	PointWiseMult();
...
// Split
__global__ void _Toom_Cook_3::Split()
{
...
// Evaluation
__global__ void _Toom_Cook_3::Evaluation()
{
	Evaluating(1stNumberSplit, 1stPolynomial);
	Evaluating(2ndNumberSplit, 2ndPolynomial);
}
// PointWiseMult
__global__ void _Toom_Cook_3::PointWiseMult()
{
...
// Interpolation
__global__ void _Toom_Cook_3::Interpolation()
{
...
// Recomposition
__global__ void _Toom_Cook_3::Recomposition()
{
}
// computeLength
...
// leftSplit
__global__ ulong _Toom_Cook_3::leftSplit(ulong number, const int& length)
{
	int middle = length / 2;
	...
	__shared__ ulong result(number);
    int remLoop(0);
...
// rightSplit
__global__ ulong _Toom_Cook_3::rightSplit(ulong number, const int& length)
{
	ulong remainder(0), multiply(1);
	__shared__ ulong result(0);
